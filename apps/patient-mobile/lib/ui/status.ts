...
/**
 * Mirrors `@referralplatform/ui-components`'s `StatusTone` union — that
 * package is web-only (DOM elements, `lucide-react` icons, CSS custom
 * properties), so this app's own `components/ui.tsx` defines a React
 * Native `StatusBadge` driven by this same tone vocabulary instead of
 * importing the web package. Keep in sync if the web tone set changes.
 */

 ...
 * policy denial — 403 on CONNECT — not a transient failure). Without a
 * generated client, `import { PrismaClient } from '@prisma/client'` throws at
 * module load time, which would make every test that transitively imports
 * PrismaService impossible to run in this sandbox, even ones that never
 * touch Postgres. See BUILD_LOG/directory.md for the full story (mirrors the
 * identical, already-documented workaround in services/audit-log,
 * services/identity-access and services/gp-authorisation).
 *
 ...
 * calls it actually makes (see src/directory/directory.service.spec.ts and
 * src/secure-messaging/secure-messaging.service.spec.ts) — this stub's
 * `$connect`/`$disconnect` are never expected to do anything real.
 * Delete the `moduleNameMapper` entry in jest.config.js once
 * `npm run prisma:generate -w services/directory` can run for real.
 */

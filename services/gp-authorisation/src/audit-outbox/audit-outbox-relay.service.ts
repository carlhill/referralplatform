 ...
 * `GpLink` state change, and calls the real Audit Log Service for each. A
 * row is only marked published after `auditClient.record()` succeeds, so a
 * transient Audit Log Service outage just delays publication (retried every
 * tick) rather than losing the event — the domain write and the fact that
 * it *needs* auditing are never lost, only the timing of the actual signed
 * write is.
 */
@Injectable()

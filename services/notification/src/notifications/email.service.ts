...
/**
 * REAL email delivery via nodemailer/SMTP — in every environment
 * configured by this repo's `docker-compose.yml` and `.env.example`, that
 * SMTP server is Mailhog (`SMTP_HOST=mailhog`, `SMTP_PORT=1025`): a real,
 * working SMTP send, just to a local mail-catcher instead of a real inbox
 * (visible at http://localhost:8025). Per
 * `claude/modules-and-requirements.md`: "the SMS provider is a mock ... the
 * OTP/account-activation channel runs on real email delivery instead so
 * onboarding is actually testable end-to-end." This is the generic email
 * channel of the Notification Service's push/SMS/email fan-out — any
 * service can call `POST /notifications/email` (or `/dispatch` with an
 * email fallback) to reach it; it is also used directly for this service's
 * own OTP/account-activation convenience helpers below.
 *
 * Swapping in a real transactional-email provider (SES, SendGrid, ...) for
 * production is a `nodemailer.createTransport(...)` config change, not a
 * redesign — every call site in this service goes through this one class.
 */
@Injectable()
export class EmailService {
  ...
  async send(input: SendEmailInput): Promise<SendEmailResult> {
    try {
      const info = await this.transporter.sendMail({
        ...
        text: input.text,
        html: input.html ?? escapeHtml(input.text).replace(/\n/g, '<br/>'),
      });
      return { providerMessageId: info.messageId };
    } catch (err) {
      this.logger.error(`Failed to send email to ${redactEmail(input.to)}: ${(err as Error).message}`);
      throw err;
    }
  }

  /** Convenience helper — OTP codes, per this service's role as the real OTP-email channel. */
  ...
  /** Convenience helper — account-activation link, per this service's role as the real activation-email channel. */
  async sendActivationLinkEmail(input: {
    to: string;

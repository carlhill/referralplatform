 ...
 *
 * Uses `@nestjs/schedule`'s `@Interval` rather than a cron expression — this
 * needs to run frequently (every few seconds) so an account-activation flow
 * that depends on "did the audit write succeed" (none currently block on
 * it, by design) never waits long, not on a calendar schedule.
 */
@Injectable()

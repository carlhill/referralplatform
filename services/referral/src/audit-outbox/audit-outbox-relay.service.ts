 ...
 * `Referral` state change or `ComplianceFlag` decision, and calls the real
 * Audit Log Service for each. A row is only marked published after
 * `auditClient.record()` succeeds, so a transient Audit Log Service outage
 * just delays publication (retried every tick) rather than losing the event.
 */
@Injectable()

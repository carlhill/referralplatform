 ...
 * touch Postgres. See BUILD_LOG/referral.md and BUILD_LOG/gp-authorisation.md
 * for the full story (an identical, already-documented workaround).
 *
 ...
 * calls it actually makes (see src/referral/referral.service.spec.ts and
 * src/compliance-rules/compliance-rules.service.spec.ts) — this stub's
 * `$connect`/`$disconnect` are never expected to do anything real.
 * Delete the `moduleNameMapper` entry in jest.config.js once
 * `npm run prisma:generate -w services/referral` can run for real.
 */

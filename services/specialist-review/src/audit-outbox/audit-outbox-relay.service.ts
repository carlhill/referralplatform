...
/**
 * The relay half of the outbox pattern (root CONVENTIONS.md §7) — identical
 * pattern to services/referral/src/audit-outbox/audit-outbox-relay.service.ts.
 * Reads unpublished `AuditOutbox` rows written in the same DB transaction as
 * a case-lifecycle write (extraction confirmed, branch decided, pathology
 * requested, case completed/cancelled) and calls the real Audit Log Service
 * for each. A row is only marked published after `auditClient.record()`
 * succeeds, so a transient Audit Log Service outage just delays publication
 * (retried every tick) rather than losing the event.
 */
@Injectable()

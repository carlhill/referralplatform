 ...
 * policy denial, not a transient failure). Without a generated client,
 ...
 * touch Postgres. Identical, already-documented workaround as
 * services/referral and services/gp-authorisation — see their BUILD_LOGs.
 *
 ...
 * like the calls it actually makes (see src/cases/cases.service.spec.ts) —
 * this stub's `$connect`/`$disconnect` are never expected to do anything
 * real. Delete the `moduleNameMapper` entry in jest.config.js once
 * `npm run prisma:generate -w services/specialist-review` can run for real.
 */
